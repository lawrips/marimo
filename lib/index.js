...
const PortFinder = require('./modules/utils/portFinder'),
    ...
    fileHelper = require('./modules/utils/fileHelper');
...
class Marimo {
            ...
            ws.on('message', (data, flags) => {                
                ...
                try {                    
                    ...
                    if (this.env !== false && this.env !== 'false') {
                        if (typeof message.env === 'object' || !message.env) {
                            // if process.env is an object
                            // merge them with environment variables
                            env = Object.assign(env, message.env);
                        }
                        else {
                            try {
                                // it's a string - try to load it as a file
                                let regex = new RegExp(/^(.+?)(\.[^.]*$|$)/);
                                if (!this.environments[message.env.match(regex)[1]]) {
                                    let error = `couldn't find environment ${message.env}, check the availableEnvironments object when first connecting`;
                                    debug(error)
                                    return ws.send(error);
                                }
                                // find the full path name
                                env.marimo_env_file = path.resolve(this.environments[message.env.match(regex)[1]].file);
                            } catch (ex) {
                                let error = `error loading environment variables file ${process.marimoEnv}: ${ex}`;
                                debug(error)
                                return ws.send(error);                                
                            }
                        }
                    ...
                    if (message.monitor) {
                        if (message.monitor.cmd == 'start') {                            
                            ...
                            async.map(message.test.split(','), (test, mapCallback) => {
                                ...
                                this._run(this.monitorWs, env, test, message.reporter, message.monitor.delay || 5000, (err) => {
                        ...
                        // now start test                         
                        async.map(message.test.split(','), (test, mapCallback) => {
                            this._run(ws, env, test, message.reporter, null, (err) => {
                                if (err) debug(`error while running ${test}: ${err}`);
                                debug(`finished running ${test}`);
                                return mapCallback();
                            });
    ...
    /**
     * Load a file and make it available from marimo 
     * @file {string} - The location of a file. Should be a file path relative to the server that has initialized marimo. 
     */

    addFile(file) {
        this.loader.addFile(file, (err, loaded) => {
            if (err) {
                debug(`Failed to load file ${file}: ${err}`);
                return;
            }
    ...
    _run(ws, env, test, reporter, delay, callback) {
