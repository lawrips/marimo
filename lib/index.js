...
const PortFinder = require('./modules/portFinder'),
...
class Marimo {
    ...
    constructor(options) {
        ...
        // load tests
        let loader = new Loader(this.directory);
        let loaded = loader.loadTests();
        ...
        this.portRange = options.debugPortRange || 100;        
            ...
            ws.on('message', (data, flags) => {                
                ...
                try {                    
                    ...
                    if (message.monitor) {
                        ...
                        // now start test                         
                        this._run(ws, env, message.test, message.envFile, message.reporter, null, (err) => {
                            if (err) debug(`error while running ${message.test}: ${err}`);
                            debug(`finished running ${message.test}`);
                        });
                    }
    ...
    _run(ws, env, test, envFile, reporter, delay, callback) {
        ...
        if (this.environments[envFile]) {
            env.env_file = this.environments[envFile].file;
        }
