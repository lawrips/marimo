...
class Marimo {
    ...
    constructor(options) {
        ...
        this.basePort = options.debugPort || 12141;
        this.portRange = options.debugPortRange || 50;        
        this.maxPort = options.debugPort || 12141 + this.portRange;
            ...
            ws.on('message', (data, flags) => {                
                ...
                try {                    
                    ...
                    if (message.monitor) {
                        ...
                        // now start test                         
                        this._run(ws, env, message.test, message.reporter, null, (err) => {
                            if (err) debug(`error while running ${message.test}: ${err}`);
                            debug(`finished running ${message.test}`);
                        });
                    }
    ...
    _run(ws, env, test, reporter, delay, callback) {
        ...
        // load the script
        let script = path.join(path.dirname(fs.realpathSync(__filename)), './modules/processManager.js');
